...
from src.k import CreateByWS, Create, CreateKLines
...
class KLineWatcher:
  ...
  def Start(self):
    print('开始监听')
    klines = CreateKLines(self.client.klines(self.symbol, self.interval, limit = self.limit))
    print(klines[len(klines) - 1].closed)
    self.wsClient.start()
    self.wsClient.kline(
      symbol = self.symbol,
      id = 1,
